...
int Tableau::getIndice(const string& nom) const {
    for (int i = 0; i < m_nbElem; i++) {
        if (m_tableau[i].getNom() == nom) {
            return i;
        }
...
bool Tableau::operator==(const Tableau& autreTableau) const {
    if (m_nbElem != autreTableau.m_nbElem) {
        return false;
    }

    for (int i = 0; i < m_nbElem; i++) {
        if (m_tableau[i] != autreTableau.m_tableau[i]) {
            return false;
        }
    }
